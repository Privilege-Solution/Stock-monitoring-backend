...
function classifyEventType(remark) {
  ...
  if (/โครงการ|เปิดโครงการ|ยอดจอง|openday|site visit|oppday/i.test(r)) return 'โครงการ';
...
function generate(data, lang = 'th') {
  ...
  const withRemark = withPrice.filter(r => r.remark && r.setIdx != null);
  if (withRemark.length) {
    ...
    if (ranked[0]?.impact > 0) bullets.push(t.event(ranked[0].r.remark));
  }
  ...
  // Rule 5: event-type wins
  const counts = {};
  for (const r of withPrice.filter(r => r.remark)) {
    const k = classifyEventType(r.remark);
